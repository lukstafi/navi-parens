...
function testCase(content: string, command: string, mode: string, language: string, debugThis?: boolean | undefined) {
	...
	return async () => {
		...
		const commands = new Map(Object.entries({
			...
			'goUpOneLiner': () => myExtension.goOneLiner(textEditor, false, true),
			'goDownOneLiner': () => myExtension.goOneLiner(textEditor, false, false),
			'selectUpOneLiner': () => myExtension.goOneLiner(textEditor, true, true),
			'selectDownOneLiner': () => myExtension.goOneLiner(textEditor, true, false),
